#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""CommandLineGAN.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/11fkzNnT7ro5mlLr8oHg9h0HiaUUWfIV9

Getting all of the imports set up and picking a seed for the random number generator

Following the tutorial from [here](https://pytorch.org/tutorials/beginner/dcgan_faces_tutorial.html)

All explanation around the code is copied from the tutorial for the purpose of better understanding the things going on.
"""
...
def runNetwork(datasetChoice, eps):
    outfile = open('console.log', 'w')
    ...
    **workers** - the number of worker threads for loading the data with the DataLoader (default 2)

    **batch_size** - the batch size used in training. The DCGAN paper uses a batch size of 128 (default 128)

    **image_size** - the spatial size of the images used for training. This implementation defaults to 64x64. If another size is desired, the structures of D and G must be changed. [See here for more details](https://github.com/pytorch/examples/issues/70) (default 64)

    **nc** - number of color channels in the input images. For color images this is 3 (default 3)

    **nz** - length of latent vector (default 100)

    **ngf** - relates to the depth of feature maps carried through the generator (default 64)

    **ndf** - sets the depth of feature maps propagated through the discriminator (default 64)

    **num_epochs** - number of training epochs to run. Training for longer will probably lead to better results but will also take much longer (default 5)

    **lr** - learning rate for training. As described in the DCGAN paper, this number should be 0.0002 (default 0.0002)

    **beta1** - beta1 hyperparameter for Adam optimizers. As described in paper, this number should be 0.5 (default 0.5)

    **ngpu** - number of GPUs available. If this is 0, code will run in CPU mode. If this number is greater than 0 it will run on that number of GPUs (default 1)
    """

    # file to save the resulting networks
    #outdir = "/content/drive/My Drive/GANdata/Networks/"

    # Root directory for dataset
    #datasetChoice = 'pokemon'
    dataroot = "../../datasets/{}".format(datasetChoice)
    ...
    # Create the dataset
    dataset = dset.ImageFolder(root=dataroot, #specify the folder of folders of pictures
                               transform=transforms.Compose([ #create a sequence of transformations to put the images through
    ...
    netD = Discriminator(ngpu, nz, ngf, ndf, nc).to(device)
    # Handle multi-gpu if desired
    if (device.type == 'cuda') and (ngpu > 1):
    ...
    **Note:** This step might take a while, depending on how many epochs you run and if you removed some data from the dataset.
    ...
    outfile.write("Starting Training Loop...")
    # For each epoch
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            if i % 50 == 0:
                outfile.write('[%d/%d][%d/%d]\tLoss_D: %.4f\tLoss_G: %.4f\tD(x): %.4f\tD(G(z)): %.4f / %.4f\n' % (epoch, num_epochs, i, len(dataloader),errD.item(), errG.item(), D_x, D_G_z1, D_G_z2))
    ...
    **Loss versus training iteration**
    ...
    """![Plot](https://pytorch.org/tutorials/_images/sphx_glr_dcgan_faces_tutorial_002.png)
    ...
    **Visualization of G’s progression**
    ...
    """
    with open('timings.pkl', 'wb') as timingsOutput:
        pickle.dump(timings, timingsOutput)
    ...
    """Code to attempt to save my networks... lets hope this works"""
    ...
    outfile.close()
    plt.close('all')
    return min(G_losses), min(D_losses) 

if __name__ == '__main__':
    datasetChoice = 'pokemon'
    eps = 5
    runNetwork(datasetChoice, eps)
